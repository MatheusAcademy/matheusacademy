...
function buildSidebar(){
  ...
  if(btnCont&&!isNaN(lastMi)&&MODS[lastMi]){
    ...
    btnCont.innerHTML='<svg style="width:14px;height:14px;opacity:.6;flex-shrink:0" viewBox="0 0 24 24" fill="currentColor" stroke="none"><path d="M8 5v14l11-7z"/></svg> Continuar M\u00f3dulo '+(lastMi+1);
...
/* ── Extrai texto COMPLETO do tópico (1 string única) ── */
function _mlExtrairTexto(htmlStr,prefixo){
  ...
  tmp.querySelectorAll('h1,h2,h3,h4,h5,h6,p,li').forEach(function(el){
    ...
    if(t.length>5)partes.push(t);
  });
  return partes.join(' ');
}

/* ── Extrai blocos individuais para mapear posição do clique ── */
...
function _mlPlayUrl(blobUrl,spd,mi,onEnd){
  if(_mlAudioEl){try{_mlAudioEl.pause();_mlAudioEl.src='';}catch(e){}}_mlAudioEl=null;
...
function mlToggleModAudio(mi){
  ...
  // Tocando este módulo/tópico → pausa/retoma
  if(_mlAudioEl&&_mlModMi===mi){
    if(!_mlAudioEl.paused){
      _mlAudioEl.pause();_mlSetBtn(mi,'paused');
    } else {
      _mlAudioEl.playbackRate=spd;_mlAudioEl.play();_mlSetBtn(mi,'playing');
    }
    return;
  }
  ...
  // Acha tópico aberto
  var mod=MODS[mi],topicoAberto=null,topicoIdx=-1;
  mod.topics.forEach(function(t,ti){
    var body=document.getElementById('ml_tbody_'+mi+'_'+ti);
    if(body&&body.offsetHeight>0&&body.style.display!=='none'){topicoAberto=t;topicoIdx=ti;}
  });
  ...
  if(!topicoAberto){
    ...
    _mlFetchTTS(txtMod,_mlAbort.signal).then(function(url){
      _mlLoading=false;_mlAbort=null;_mlPlayUrl(url,spd,mi,null);
    }).catch(function(e){if(e.name!=='AbortError'){_mlLoading=false;_mlSetBtn(mi,'idle');}});
    return;
  }
...
function _mlBindTopicClicks(mi,ti){
  ...
  els.forEach(function(el,idx){
    ...
    el.addEventListener('click',function(e){
      // Só narra se o áudio já estiver ativo neste módulo (tocando ou pausado)
      // Se o áudio não estiver ativo, o clique é ignorado (deixa o tópico expandir/recolher normalmente)
    ...
    el.addEventListener('mouseenter',function(){
      if(_mlModMi===mi&&(_mlAudioEl||_mlLoading)){
        ...
        el.style.background='rgba(91,127,255,0.08)';
        el.style.borderRadius='4px';
        el.style.outline='1px solid rgba(91,127,255,0.2)';
      } else {
        el.style.cursor='';
        el.title='';
        el.style.background='';
...
/* ═══ MINI PALETTE — selecione texto → 3 cores + remover ═══ */
var _toolbar=null,_hlPalette=null,_notePopup=null,_curRange=null,_pendingHlId=null;
var _hlPaletteVisible=false,_selDebounce=null;
...
function initHighlight(){
  ...
  /* Click handlers nos dots de cor */
  _hlPalette.querySelectorAll('.hl-mp-dot').forEach(function(dot){
    dot.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();applyHighlight(this.dataset.hl);});
    dot.addEventListener('touchend',function(e){e.preventDefault();e.stopPropagation();applyHighlight(this.dataset.hl);});
  });
  /* Botão remover */
  var rb=_hlPalette.querySelector('.hl-mp-remove');
  if(rb){
    rb.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();removeHighlight();});
    rb.addEventListener('touchend',function(e){e.preventDefault();e.stopPropagation();removeHighlight();});
  }
  /* selectionchange — funciona em todos os dispositivos */
  document.addEventListener('selectionchange',function(){
    clearTimeout(_selDebounce);
    _selDebounce=setTimeout(_checkSel,200);
  });
  /* mouseup backup para desktop */
  document.addEventListener('mouseup',function(e){
    if(_hlPalette&&_hlPalette.contains(e.target))return;
    setTimeout(_checkSel,50);
  });
  /* Esconder ao clicar fora */
  document.addEventListener('mousedown',function(e){
    if(_hlPalette&&!_hlPalette.contains(e.target))hideToolbar();
    if(_notePopup&&!_notePopup.contains(e.target))closeHlNote();
  });
...
function hideToolbar(){
  if(_hlPalette){_hlPalette.classList.remove('visible');_hlPaletteVisible=false;}
...
function addNoteToHighlight(){
  ...
  if(_hlPalette){var palRect=_hlPalette.getBoundingClientRect();_notePopup.style.top=(palRect.bottom+window.scrollY+8)+'px';_notePopup.style.left=palRect.left+'px';}
...
function extractFirstParagraph(htmlStr,maxLen){
  maxLen=maxLen||300;
  if(!htmlStr)return '';
  var tmp=document.createElement('div');
  tmp.innerHTML=htmlStr;
  ...
  var txt='';
  for(var i=0;i<ps.length&&txt.length<maxLen;i++){
...
function countStats(mods){
  var totalTopics=0,totalCards=0,totalQuiz=0,totalWords=0;
  mods.forEach(function(m){
    m.topics.forEach(function(t){
      totalTopics++;
      if(t.cards)totalCards+=t.cards.length;
      if(t.html)totalWords+=extractTextFromHtml(t.html).split(/\s+/).length;
    });
    if(m.quizFinal)totalQuiz+=m.quizFinal.length;
  });
...
function generateFullMindMapPDF(){
  showToast('⏳ Gerando PDF completo do mapa mental...','info');

  var cc=COURSE.color||'#4A7EFF';
  var now=new Date();
  var exportDate=now.toLocaleDateString('pt-BR',{day:'2-digit',month:'long',year:'numeric'});
  ...
  var statsData=[
    ...
    {icon:'⏱️',value:COURSE.hours,label:'Horas',color:'#f59e0b'},
    {icon:'🎴',value:stats.cards,label:'Flashcards',color:'#8b5cf6'},
    {icon:'❓',value:stats.quiz,label:'Questões',color:'#ec4899'},
...
setInterval(function(){
  ...
  var el=document.getElementById('stTime');if(el){el.textContent=m+':'+s;el.closest('.study-timer')&&(el.closest('.study-timer').style.display='none');}
