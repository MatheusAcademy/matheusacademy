...
function selectTopic(mi,ti,canAcc){
  ...
  setTimeout(function(){
    ...
    if(item&&!item.classList.contains('t-open')){
      ...
      // Já está aberto — rola para o cabeçalho
...
function mlToggleTopic(mi,ti){
  ...
  if(!isOpen){
    ...
    _curModIdx=mi;_curTopIdx=ti;
    // Rola para o TOPO do cabeçalho do tópico (não para o conteúdo)
    setTimeout(function(){
      var hdr=item.querySelector('.ml-topic-hdr');
      var target=hdr||item;
      var topbarH=56; // altura da topbar fixa
      var tabsH=48;   // altura das tabs
